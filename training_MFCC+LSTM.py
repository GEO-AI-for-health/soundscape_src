...
class SGDLearningRateTracker(Callback):
    ...
    def loss_plot(self, loss_type):
        ...
        myfig.savefig(
            r'bestmodels_2rnn/q4_02_1-3_8-10lstm/' + str(
                n_fold) +
...
for train_index, val_index in k_fold.split(train_X, train_Y):
    ...
    # build model
    model = Sequential()
    model.add(LSTM(units=lstm_units, return_sequences=True, activation='tanh', recurrent_activation='hard_sigmoid',
                   input_shape=(X_train.shape[2], X_train.shape[1])))
    print(X_train.shape)
    print(X_train.shape[2], X_train.shape[1])
    model.add(Dropout(0.5))
    model.add(LSTM(units=lstm_units, activation='tanh', recurrent_activation='hard_sigmoid'))
    model.add(Dropout(0.5))
    ...
    save_best_model = ModelCheckpoint(
        'bestmodels_2rnn/q4_02_1-3_8-10lstm/'
        + str(n_fold) + '_' + 'lstm_weights.{epoch:02d}-{acc:.4f}-{val_acc:.4f}.hdf5',
        monitor='val_acc',
